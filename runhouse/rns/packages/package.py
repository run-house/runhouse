...
class Package(Resource):
    ...
    def config_for_rns(self):
        ...
        config['install_target'] = self._resource_string_for_subconfig(self.install_target)
    ...
    def install(self):
        ...
        if isinstance(self.install_target, Folder):
            ...
            if self.install_method == 'pip':
                ...
                if (Path(local_path) / 'requirements.txt').exists():
                    ...
                    self.pip_install(f'-r {Path(local_path)}/requirements.txt' + install_args)
                else:
        ...
        if self.install_method == 'pip':
            ...
            if isinstance(self.install_target, Folder):
                ...
                raise ValueError(f'install_target must be a Folder or a path to a directory for '
        ...
        #         sys.modules[self.name] = pickle.load(f)
            ...
            raise ValueError(f'Unknown install_method {self.install_method}. Try using cluster.run() or '
    ...
    def conda_install(install_cmd: str):
        ...
        try:
            ...
            subprocess.check_call('wget https://repo.continuum.io/miniconda/Miniconda3-latest-Linux-x86_64.sh '
            ...
            status = subprocess.check_call(['conda', 'install', '-y'] + install_cmd.split(' '))
            if not status == 0:
...
def package(name=None,
    ...
    if url is not None:
        config['install_target'] = Folder(url=url, fs=fs, local_mount=local_mount, data_config=data_config)
