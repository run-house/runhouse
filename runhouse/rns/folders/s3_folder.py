...
class S3Folder(Folder):
    ...
    def delete_in_fs(self, recurse=True, *kwargs):
        ...
        try:
            ...
            S3Store(name=self.bucket_name_from_url(self.url), source=self._fsspec_fs).delete()
    ...
    def upload(self, src: str, region: Optional[str] = None):
        ...
        s3_store = S3Store(name=self.bucket_name_from_url(self.url), source=src, region=region)

        sync_dir_command = self.upload_command(src=src, dest=self.url)
        self.run_upload_cli_cmd(sync_dir_command, access_denied_message=s3_store.ACCESS_DENIED_MESSAGE)
