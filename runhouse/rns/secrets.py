...
class Secrets:
    ...
    def extract_and_upload(cls,
        ...
        if resp.status_code != 200:
            raise Exception(f'Failed to update secrets in Vault: {json.loads(resp.content)}')
    ...
    def download_into_env(cls,
        ...
        resp = requests.get(f'{rns_client.api_server_url}/{cls.USER_ENDPOINT}',
                            headers=headers or rns_client.request_headers)
        if resp.status_code != 200:
    ...
    def put(cls,
        ...
        resp = requests.put(f'{rns_client.api_server_url}/{endpoint}/{provider}',
    ...
    def enabled_providers(cls, as_str: Optional[bool] = False) -> List:
        ...
        if as_str:
            return [c.PROVIDER_NAME for c in cls.__subclasses__() if c.PROVIDER_NAME in cloud_names]

        return [c for c in cls.__subclasses__() if c.PROVIDER_NAME in cloud_names]

    @classmethod
    def set_endpoint(cls, group: Optional[str] = None):
        return f'{cls.GROUP_ENDPOINT}/{group}' if group is not None else cls.USER_ENDPOINT
...
class AWSSecrets(Secrets):
    ...
    def read_secrets(cls, from_env: bool = False, file_path: Optional[str] = None):
        if from_env:
            ...
            if not access_key or not secret_key:
                raise Exception(f"AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY must be set for {cls.PROVIDER_NAME}")
        else:
            if file_path:
        ...
        return {'provider': cls.PROVIDER_NAME,
    ...
    def save_secrets(cls, secrets: dict, file_path: Optional[str] = None):
        ...
        parser.set(section=section_name, option='aws_access_key_id', value=secrets['access_key'])
        parser.set(section=section_name, option='aws_secret_access_key', value=secrets['secret_key'])
...
class AZURESecrets(Secrets):
    ...
    def read_secrets(cls, from_env: bool = False, file_path: Optional[str] = None):
        if from_env:
            ...
            if not subscription_id:
                raise Exception(f"AZURE_SUBSCRIPTION_ID must is not set for {cls.PROVIDER_NAME}")
        else:
    ...
    def save_secrets(cls, secrets: dict, file_path: Optional[str] = None):
        ...
        parser.add_section(section_name)
        parser.set(section=section_name, option='subscription', value=secrets['subscription_id'])
...
class GCPSecrets(Secrets):
    PROVIDER_NAME = 'gcp'
    CREDENTIALS_FILE = os.path.expanduser('~/.config/gcloud/application_default_credentials.json')
    ...
    def read_secrets(cls, from_env: bool = False, file_path: Optional[str] = None):
        if from_env:
            ...
            if not client_id or not client_secret:
                raise Exception(f"CLIENT_ID and CLIENT_SECRET must be set for {cls.PROVIDER_NAME}")
        else:
            creds_file = file_path or cls.CREDENTIALS_FILE
...
class HUGGINGFACESecrets(Secrets):
    ...
    def read_secrets(cls, from_env: bool = False, file_path: Optional[str] = None):
        if from_env:
            raise NotImplementedError(f"Reading secrets from env is not supported for {cls.PROVIDER_NAME}")
        else:
...
class SKYSecrets(Secrets):
    ...
    @classmethod
    def has_secrets_file(cls) -> bool:
        return cls.file_exists(cls.PRIVATE_KEY_FILE) and cls.file_exists(cls.PUBLIC_KEY_FILE)
    ...
    def save_secrets(cls, secrets: dict, file_path: Optional[str] = None):
        ...
        sky.authentication._save_key_pair(private_key_path,
                                          public_key_path,
