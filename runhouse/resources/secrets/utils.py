...
def setup_cluster_creds(ssh_creds: Union[Dict, str], resource_name):
    ...
    if len(creds_keys) == 1 and "ssh_private_key" in creds_keys:
        if Path(ssh_creds["ssh_private_key"]).expanduser().exists():
            values = SSHSecret._from_path(self=SSHSecret, path=ssh_creds["private_key"])
        ...
        if private_key_path.exists() and public_key_path.exists():
            if private_key_path.parent == public_key_path.parent:
                values = SSHSecret._from_path(self=SSHSecret, path=private_key)
            else:
                values = SSHSecret._from_path(
                    self=SSHSecret, path=private_key, public_path=public_key
                )
        ...
        if Path(private_key).expanduser().exists():
            private_key = SSHSecret._from_path(self=SSHSecret, path=private_key).get(
                "private_key"
            )
        if Path(username).expanduser().exists():
            username = ProviderSecret._from_path(username)
        values = {
        ...
        if Path(password).expanduser().exists():
            password = ProviderSecret._from_path(password)
        if Path(username).expanduser().exists():
            username = ProviderSecret._from_path(username)
