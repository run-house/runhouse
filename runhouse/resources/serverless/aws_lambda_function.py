...
class AWSLambdaFunction(Function):
    ...
    def _create_layer_zip(self):
        ...
        cwd = os.getcwd()
        all_req_dir = Path(os.path.join(cwd, "all_reqs"))
        all_req_dir.mkdir(exist_ok=True, parents=True)

        all_req_dir = Path(os.path.join(all_req_dir, "python"))
        all_req_dir.mkdir(exist_ok=True, parents=True)
    ...
    def _invoke(self, *args, **kwargs) -> Any:
        ...
        try:
            ...
            log_lines = (
                base64.b64decode(invoke_res["LogResult"]).decode("utf-8").split("\n")
            )
