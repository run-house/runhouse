...
class SparkDistributed(Supervisor):
    def __init__(
        self,
        name,
        module: Module = None,
        ray_init_options=None,
        spark_init_options=None,
        **kwargs
    ):
        if not hasattr(module, "fn_pointers"):
            raise ValueError(
                "Spark Distributed requires a Runhouse Function object to distribute."
            )

        print("initializing spark distribution")
        super().__init__(name=name, **kwargs)
        self._module = module
        self._ray_init_options = ray_init_options or {}
        self._spark_init_options = spark_init_options or {}

    def _compute_signature(self, rich=False):
        return self.local._module.signature(rich=rich)

    def forward(self, item, *args, **kwargs):
        from runhouse.resources.distributed.utils import subprocess_raydp_fn_call_helper
        ...
        with multiprocessing.Pool(processes=1) as pool:
            result = pool.apply_async(
                subprocess_raydp_fn_call_helper, args=subproc_args
            )
            while True:
                try:
