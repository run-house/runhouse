...
class DistributedQueue(Supervisor):
    def __init__(self, name, replicas: List[Module] = None, max_concurrency = 1, **kwargs):
        super().__init__(name=name, **kwargs)
        self._replicas = replicas or []
        self._max_concurrency = max_concurrency
        self._available_replicas = list(range(len(self._replicas) * self._max_concurrency))
    ...
    def forward(self, item, timeout: Optional[int] = None, *args, **kwargs):
        ...
        while not self._available_replicas:
            ...
            if timeout is not None and time_waited >= timeout:
                raise TimeoutError(
                    "Timed out waiting for a replica to be available."
                )
            time.sleep(0.25)
            time_waited += 0.25
    ...
    def __call__(self, *args, **kwargs):
        if isinstance(self._replicas[0], Function):
            ...
            raise NotImplementedError("DistributedQueue.__call__ can only be called on Function replicas.")
