...
async def main():
    ...
    certs_addresses = cluster_config.get("ips", None) or cluster_config.get(
        "compute_properties", {}
    ).get("ips", None)
    ...
    if not daemon_port or daemon_port in [DEFAULT_HTTP_PORT]:
    ...
    logger.info(
        f"Launching Runhouse API server on host={host} and use_https={use_https} and port_arg={daemon_port}"
    )
