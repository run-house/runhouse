...
def get_resource_link_in_den_ui(cluster_name: str, ui_link: str):
    from runhouse.globals import rns_client

    cluster_uri = rns_client.format_rns_address(cluster_name)
    link_to_den_dashboard = f"{ui_link}/resources/{cluster_uri}"
...
def print_status(status_data: dict, current_cluster) -> None:
    ...
    if cluster_name:
        ui_link = "https://www.run.house/"
        cluster_link_in_den_ui = get_resource_link_in_den_ui(
            cluster_name=cluster_name, ui_link=ui_link
        )
...
def print_bring_cluster_up_msg(
    ...
    console.print(
        f"{msg_prefix} because [reset]{cluster_name} is not up. To bring it up, run [bold italic green]runhouse cluster up {cluster_name}[/bold italic green]."
    )
