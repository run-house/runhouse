...
class EmbeddingMapper:
    async def embed_url_recursively(self, url: str, replicas, max_depth: int = 5):
        ...
        for i, replica in enumerate(replicas):
            parallel_calls.append(replica.embed_docs(chunked_docs[i]))
...
async def main():
    ...
    replicas = []
    for i in range(3):
        env = _get_env_with_name(f"langchain_embed_env_{i}")
        remote_doc_embedder = rh.module(DocEmbedder, name=f"doc_embedder_{i}")().to(
            system=cluster, env=env
        )
        replicas.append(remote_doc_embedder)

    remote_module = rh.module(EmbeddingMapper)().to(system=cluster, env=og_env)
    await remote_module.embed_url_recursively(
        "https://js.langchain.com/docs/", replicas
    )
